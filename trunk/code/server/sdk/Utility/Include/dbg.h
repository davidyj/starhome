/*
 *XXX: Currently, this file has not been ported to FreeBSD completely due to _Crt*() routines.
 *     These routines have tight coupling to vc++'s runtime lib.
 * lujd 2009.7.1
 */
...
#else
#define DEBUG_Log NullLog
inline void NullLog( char const *pMsg, ... ) {}
#endif

/* These are always compiled in */
void Msg( char const* pMsg, ... );
void MonitorMsg( char const* pMsg, ... );
...
//void See(char const* pMsg,...);	// Write By David yang Server Monitor
...
/*
typedef void (*_Mem_Report)(void);

class MEMREPORTITEM : public CHAINITEM
{
public:
	_Mem_Report pFn;
};

static CHAINHEADER s_MemreportList;

#define IMPLEMENT_MEMREPORT( lib ) void MemReport_##lib( ){ _CrtDumpMemoryLeaks( ); } \
		class Mem__##lib \
		{ \
		public: \
			Mem__##lib( ) \
			{ \
				MEMREPORTITEM * pItem = new MEMREPORTITEM; \
				pItem->pFn = MemReport_##lib; \
				s_MemreportList.Add( pItem ); \
			} \
		}; \
		Mem__##lib g_Mem##lib; \
...
#define DECLARE_MEMREPORT( lib ) void MemReport_##lib( );

#ifdef WIN32
#define IMPLEMENT_MEMREPORT( lib ) void MemReport_##lib( ){ _CrtDumpMemoryLeaks( ); }
#else
#define IMPLEMENT_MEMREPORT( lib )
#endif

#define RUN_MEMREPORT( lib ) MemReport_##lib( );
...
#ifndef WIN32	//	2010-11-29	li9chuan

#define lstrlen strlen
static int MessageBox( HWND hWnd, LPCSTR lpText, LPCSTR lpCaption, UINT uType )
{
	return IDABORT;
}

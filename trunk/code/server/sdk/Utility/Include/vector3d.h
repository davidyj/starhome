...
//	Vector(const float *pFloat);
	...
	inline vec_t GetX( ){ return x; }
	inline vec_t GetY( ){ return y; }
	inline vec_t GetZ( ){ return z; }

	inline vec_t SetX( float sx ){ return x = sx; }
	inline vec_t SetY( float sy ){ return y = sy; }
	inline vec_t SetZ( float sz ){ return z = sz; }
...
inline bool Vector::operator==( Vector const& src ) const
{
	CHECK_VALID(src);
	CHECK_VALID(*this);
	return (abs(src.x - x) < 0.001) && (abs(src.y - y) < 0.001) && (abs(src.z - z) < 0.001);
}
...
//-----------------------------------------------------------------------------
// Quaternion : 四元数,通常和 欧拉角 一起表示物体的旋转
// remark by david 2010-12-24 11:57
//-----------------------------------------------------------------------------

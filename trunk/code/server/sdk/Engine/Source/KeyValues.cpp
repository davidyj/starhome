...
//by  to FIX mem leak  for studio engine //FIXME: ....
ENGINE_GLOBAL void MemScratchEngineRelease( )
{
	MemReleaseScratch( );
...
static unsigned char ENCRYPT_CODE[10] = { 0xe1, 0xf1, 0x39, 0x19, 0x80, 0x08, 0xc6, 0x03, 0x19, 0x8a };
...
* 注    意 : * 修改日期     修改人    修改内容
*******************************************************************************/
static void MyStrEncrypt( char* str, int nlen )
{
...
/*******************************************************************************
* 函数名称 : KeyValues::LoadFromFile
* 功能描述 : 从文件读取属性
* 参　　数 : IBaseFileSystem *filesystem : 文件系统
* 参　　数 : const char *resourceName	 : 文件名
* 参　　数 : const char *pathID			 : 文件路径ID
* 返 回 值 : bool
* 作　　者 : david
* 设计日期 : 2010年12月23日
* 注    意 : * 修改日期     修改人    修改内容
...
bool KeyValues::LoadFromFile( IBaseFileSystem *filesystem, const char *resourceName, const char *pathID )
{
	...
	filesystem->Read(buffer, fileSize,0, f); // read into local buffer
	...
	//by  decrypt mat
	if ( strstr( resourceName, ".mat"))
	{
		MyStrEncrypt(buffer, fileSize );
	}
...
//-----------------------------------------------------------------------------
// Purpose: Save the vmt to bit disk file
//			只有一层
//-----------------------------------------------------------------------------
void KeyValues::VmtSaveToBitFile(IBaseFileSystem *filesystem, FileHandle_t f )
{

	// 只有一层
	if ( !m_pSub || m_pSub->m_pSub )
		return;
	//header
	int len;
	len = strlen(GetName( ));
	filesystem->Write(&len, 2, f);
	filesystem->Write(GetName(), len, f);
	//count
	int nCount = 0;
	KeyValues *dat = NULL;
	...
	for ( dat = m_pSub; dat != NULL; dat = dat->m_pPeer )
	{
		...
		if ( dat->m_sValue && *(dat->m_sValue) )
		{
			...
			//buf.Printf( "\"%s\"\t\t\"%s\"\n", dat->GetName(), dat->m_sValue);
			//keyname
			len = strlen(dat->GetName());
			filesystem->Write(&len, 2, f);
			filesystem->Write(dat->GetName(), len, f);

			//keydatatype and keyvalue
			filesystem->Write(&dat->m_iDataType, 1, f);
			switch(dat->m_iDataType) 
			{
				case TYPE_FLOAT:
					filesystem->Write(&dat->m_flValue, 4, f);
					break;
				case TYPE_INT:
					filesystem->Write(&dat->m_iValue, 4, f);
					break;
				case TYPE_STRING:
					len = strlen(dat->m_sValue);
					filesystem->Write(&len, 2, f);
					filesystem->Write(dat->m_sValue, len, f);
					break;
				default:
					Assert(0);
					continue;
...
/*******************************************************************************
* 函数名称 : KeyValues::WriteIndents
* 功能描述 : Write out a set of indenting 增加一个缩进
* 参　　数 : IBaseFileSystem *filesystem
* 参　　数 : FileHandle_t f
* 参　　数 : int indentLevel
* 返 回 值 : void
* 作　　者 : david
...
/*******************************************************************************
* 函数名称 : KeyValues::WriteIndents
* 功能描述 : writes out a set of indenting 增加缩进
* 参　　数 : CUtlBuffer& buf
* 参　　数 : int indentLevel
* 返 回 值 : void
* 作　　者 : david
...
#else
							int result = ::wcstombs(buf, dat->m_wsValue, KEYVALUES_TOKEN_SIZE);
#endif							
						if (result)
						{
							WriteIndents(filesystem, f, indentLevel + 1);
...
#else
			int result = ::wcstombs(buf, dat->m_wsValue, 512);
#endif			
			if ( result )
			{
				SetString( keyName, buf );
...
#else
			swprintf(wbuf, 64,(const wchar_t *)("%f"), dat->m_flValue);
#endif			
...
#else			
			swprintf( wbuf, 64, (const wchar_t *)("%d"), dat->m_iValue );
#endif			
			SetWString( keyName, wbuf );
			break;
		case TYPE_WSTRING:
...
bool KeyValues::LoadFromLmtBuffer( CUtlBuffer & Buffer)
{
	...
	szTemp[len] = 0;
	Buffer.Get(szTemp, len);
	SetName( szTemp );

	//subkey
	int nCount = Buffer.GetShort( );
	...
	for ( int i = 0; i < nCount; i++)
	{
		...
		szTemp[len] = 0;
		Buffer.Get(szTemp, len);
		KeyValues *dat = CreateKey( szTemp );
		dat->SetName( szTemp );

		dat->m_iDataType = (types_t)Buffer.GetUnsignedChar( );
...
bool KeyValues::LoadFromLmtBuffer( const void * pBuffer, const int fileSize)
{

	CUtlBuffer Buffer(pBuffer, fileSize);

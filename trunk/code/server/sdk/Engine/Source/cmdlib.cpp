...
//static IFileSystem* g_pFileSystem = NULL;
...
void CmdLib_FPrintf( FileHandle_t hFile, const char *pFormat, ... )
{
	...
	while ( 1 )
	{
		...
		{
			// Write the string.
			FileSystem().Write( buf.Base(), ret, hFile );


			// Write a null terminator.
			char cNull = 0;
			FileSystem().Write( &cNull, 1, hFile );
...
char* CmdLib_FGets( char *pOut, int outSize, FileHandle_t hFile )
{
	...
	{
		char c;
		if ( !FileSystem().Read( &c, 1, 0, hFile ) )
		{
			if ( iCur == 0 )
				return NULL;
			else
...
{
	Assert( (!g_pLogFile) );
	g_pLogFile = FileSystem().Open( pFilename, "a" );
...
char *ExpandGameDir(){return gamedir;};
char *ExpandBaseGameDir(){return basegamedir;};
char *ExpandQProject(){return qproject;};
char *ExpandQDir(){return qdir;};
IBaseFileSystem *GetFileSystem(){return g_pFileSystem;};
...
void SetQdirFromPath( const char *path )
{
	...
	void CmdLib_InitFileSystem( const char *pBSPFilename, bool bUseEngineFileSystem )
	{
		//FileSystem_Init( bUseEngineFileSystem );//by
		SetQdirFromPath( pBSPFilename );
	}
...
{
	return FileSystem().Size( f );
}


FileHandle_t SafeOpenWrite (char *filename)
{
	FileHandle_t f = FileSystem().OpenAbsolutePath(filename, "wb");
...
#else
	char *pFileName = ( char * )alloca( strlen( pFileName_ ) + 1 );
#endif
...
FileHandle_t SafeOpenRead( char *filename )
{
	...
	{
		if ( PATHSEPARATOR( filename[0] ) && PATHSEPARATOR( filename[1] ) )
		{
			f = FileSystem().Open( filename, "rb" );
			if ( !f )
				Error ("Error opening %s: %s",filename,strerror(errno));
		...
		{
			f = FileSystem().OpenAbsolutePath( filename, "rb" );
			if ( !f )
				Error ("Error opening %s: %s",filename,strerror(errno));
...
{
	if ( FileSystem().Read (buffer, count,0, f) != count)
...
{
	if (FileSystem().Write (buffer, count, f) != count)
...
{
	FileHandle_t hFile = FileSystem().Open( filename, "rb" );

...
template<class T,class K = UINT64,class I = UINT32>
class Item
{
public:
	Item()
...
inline bool CUtlGUIDMap<T,K,I>::init(I nSize)
{
	m_TableSize = nSize;
...
template<class T,class K,class I>
inline int CUtlGUIDMap<T,K,I>::add(K key,T* element)
{
...
template<class T,class K,class I>
inline void CUtlGUIDMap<T,K,I>::remove(K key)
{
	//每个线程都会分配这个临时变量，所以是线程安全的 david 2011-04-20
	I temp = key % m_TableSize;
...
template<class T,class K,class I>
inline T* CUtlGUIDMap<T,K,I>::get(K key)
{
	//每个线程都会分配这个临时变量，所以是线程安全的 david 2011-04-20
	I temp = key % m_TableSize;
...
template<class T,class K,class I>
inline T* CUtlGUIDMap<T,K,I>::getByIndex(I index)
{
	if(index>=0 && index<m_TableSize)
		return m_ItemTable[index].element;
	return NULL;
}

...
#define UTLLIST_ADDTOTAIL( LINKLIST,DATA, SUCCEED )				\
	{														\
	SUCCEED = TRUE;											\
	try	{	LINKLIST.AddToTail( DATA); }					\
	catch(char* p)	{ SUCCEED = FALSE;Assert( 0 && p ); UNREFERENCED_PARAMETER(p);}	\
	}														\

#define UTLLISTPTR_ADDTOTAIL( LINKLIST,DATA, SUCCEED )			\
	{														\
	SUCCEED = TRUE;											\
	try	{	LINKLIST->AddToTail( DATA );}					\
	catch(char* p){	SUCCEED = FALSE; Assert( 0 && p ); UNREFERENCED_PARAMETER(p);}	\
...
#define UTLLIST_ADDTOHEAD( LINKLIST,DATA, SUCCEED )				\
	{														\
	SUCCEED = TRUE;											\
	try	{	LINKLIST.AddToHead( DATA); }					\
	catch(char* p)	{ SUCCEED = FALSE;Assert( 0 && p );UNREFERENCED_PARAMETER(p);}	\
	}														\

#define UTLLISTPTR_ADDTOHEAD( LINKLIST,DATA, SUCCEED )			\
	{														\
	SUCCEED = TRUE;											\
	try	{	LINKLIST->AddToHead( DATA );}					\
	catch(char* p){	SUCCEED = FALSE; Assert( 0 && p ); UNREFERENCED_PARAMETER(p);}	\
...
template <class T, class I = int>//unsigned short>
class CUtlLinkedList //: public IBaseObj
{
public:
...
protected:
	...
	{
		m_Memory.SetMode(nGrowSize,nInitAllocationCount);
	}
...
I CUtlLinkedList<T,I>::AllocInternal( bool multilist )
{
	...
	if (m_FirstFree == InvalidIndex())
	{
		...
		{
			if(!m_Memory.Grow())  //by zhao 2010-7-23
				return InvalidIndex();
		}

		Assert( m_TotalElements != InvalidIndex() );
		if(m_TotalElements==InvalidIndex())
		{
			Error("CUtlLinkedList overflow!\n");
			//应该直接返回错误 By David 2010-07-19
			return InvalidIndex();
		}
...
{
	if (m_TotalElements /*m_ElementCount*/ == 0)
		return;

	// Put everything into the free list
	I prev = InvalidIndex();
	for (int i = (int)m_TotalElements /*m_ElementCount*/; --i >= 0; )
	{

...
public:
	...
	virtual int				SyncRead( void* pOutput, int size, FileHandle_t file ) = 0;
	virtual int				Read( void* pOutput, int size, int nPos, FileHandle_t file ) = 0;
...
public:
	...
	virtual bool			AddSearchPath( const char *pPath, const char *pathID, SearchPathAdd_t addType = PATH_ADD_TO_TAIL ) = 0;
	virtual void			SetFileFindOrder(bool fileOrder) = 0;
	virtual bool			RemoveSearchPath( const char *pPath, const char *pathID = 0 ) = 0;
	virtual const char *	GetSearchPath( const char *pathID ) = 0;

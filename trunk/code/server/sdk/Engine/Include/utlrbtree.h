...
inline bool WStringLessThan(const wchar_t * const &lhs,const wchar_t *const &rhs )
{
...
template <> inline bool CDefOps<const wchar_t *>::LessFunc( const wchar_t * const &lhs,const wchar_t * const &rhs ){ return WStringLessThan( lhs,rhs );}
template <> inline bool CDefOps<wchar_t *>::LessFunc( wchar_t * const &lhs,wchar_t * const &rhs ){ return WStringLessThan( lhs,rhs );}
//-------------------------------------
...
public:
	...
	//查找出关键字大于等于search，并且关键字最小的那个元素
	I  FindLarger( T const &search ) const;
...
template <class T, class I>
void CUtlRBTree<T, I>::SetMemoryMode(int nGrowSize, int nInitAllocationCount)
{
	m_Elements.SetMode(nGrowSize,nInitAllocationCount);
}
//-----------------------------------------------------------------------------
// gets particular elements
//-----------------------------------------------------------------------------
 ...
 */
template <class T, class I>
I CUtlRBTree<T, I>::FindLarger( T const &search ) const
{
	Assert( m_LessFunc );
	...
	while (current != InvalidIndex())
	{
		if (m_LessFunc( search, Element(current) )) //要查找的元素比当前的元素小
		{
			current = LeftChild(current);
			break;
		}
